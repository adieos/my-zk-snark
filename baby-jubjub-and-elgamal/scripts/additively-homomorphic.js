...
async function run() {
  ...
  const msg1 = BigInt("1234500054321"); // FILL THIS! first plaintext
  const msg2 = BigInt("5"); // FILL THIS! second plaintext
  ...
  C4 = [
    F.e(
      "6031791282275356574172562073926377572661261852449618486182628944041851140421",
    ),
    F.e(
      "8025732391497375923783555957557793657427695813752083324058581098025335250352",
    ),
  ];

  // add them
  C1res = babyJub.addPoint(C1, C3);
  C2res = babyJub.addPoint(C2, C4);

  // decrypt logic
  const helper = babyJub.mulPointEscalar(C1res, privKey);
  const negHelper = [F.sub(F.e("0"), helper[0]), helper[1]];
  const resultPoint = babyJub.addPoint(C2res, negHelper);

  // in practice, this would b bruteforced
  msgfinal = msg1 + msg2;
  const rawExpected = babyJub.mulPointEscalar(GENERATOR, msgfinal);
